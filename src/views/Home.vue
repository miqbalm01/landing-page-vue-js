<script setup lang="ts">
import 'bootstrap/dist/css/bootstrap.css';
import '@fortawesome/fontawesome-free/css/all.css';

import '../../_nuxt/boostrap-3.css'
import '../../_nuxt/boostrap-2.css'
import '../../_nuxt/boostrap.css'
import '../../_nuxt/boostrap-4.css'
import '../../_nuxt/entry.82049f6f.css'
import '../../_nuxt/OffCanvasMobileMenu.6d8c64f5.css'
import '../../_nuxt/entry.e4c421d5.js'
import '../../_nuxt/default.3edbe563.js'
import '../../_nuxt/services-classic.1400c3ac.js'

import '../../_nuxt/HeaderElement.d5db07f5.js'
import '../../_nuxt/OffCanvasMobileMenu.0c9baa2b.js'
import '../../_nuxt/FooterTwo.fd8c850b.js'
import '../../_nuxt/SearchPopup.770988b6.js'

import '../../_nuxt/GradationOne.a2432f5b.js'
import '../../_nuxt/CallToAction.911852d7.js'
import '../../_nuxt/IconBoxItemOne.5e04a354.js'
import '../../_nuxt/service.fa2df33f.js'
import '../../_nuxt/entry.e4c421d5.js'


import '../../cdn-cgi/scripts/5c5dd728/cloudflare-static/email-decode.min.js'
import '../../_nuxt/_payload.json'
import Header from '@/components/Header.vue';
import Footer from '@/components/Footer.vue';
</script>
<template>
    <div id="__nuxt">
        <div>
            <!--[-->
            <div class="main-container">
                <Header />
                <div class="brook-breadcrumb-area bg_image--16 ptb--270 pt_md--190 pb_md--100 pt_sm--190 pb_sm--100 breadcrumb-title-bar" data-black-overlay="8" style="background-image:url(https://picsum.photos/seed/picsum/1024/500);">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="breadcrumb-inner text-center">
                                    <h6 class="heading heading-h6 text-white">SERVICES</h6>
                                    <div class="bkseparator--30"></div>
                                    <h1 class="heading heading-h1 text-white line-height-1 font-80">What we do</h1>
                                    <div class="bkseparator--30"></div>
                                    <h5 class="heading heading-h5 text-white line-height-1-95">We focused on a very pragmatic modernist approach, intense contract <br> documentation, and high speed response. We’ve distinguished ourselves with the <br> projects we got built.</h5></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="brook-gradation-area ptb--150 ptb-md--80 ptb-sm--60 bg_color--1">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="brook-section-title text-center">
                                    <h6 class="heading heading-h6 theme-color">HOW WE DO</h6>
                                    <div class="bkseparator--35"></div>
                                    <h3 class="heading heading-h3">Work process</h3></div>
                                <div class="bk-gradation mt--30 mt_sm--5">
                                    <!--[-->
                                    <div class="item-grid mt--40 move-up-x wow" data-wow-delay=".13s">
                                        <div class="line"></div>
                                        <div class="dot-wrap">
                                            <div class="dot">
                                                <div class="count">1</div>
                                            </div>
                                        </div>
                                        <div class="content">
                                            <h5 class="heading heading-h5">Brainstorming</h5>
                                            <p class="bk_pra">study the problem and general materials to find out creative concepts.</p>
                                        </div>
                                    </div>
                                    <div class="item-grid mt--40 move-up-x wow" data-wow-delay=".13s">
                                        <div class="line"></div>
                                        <div class="dot-wrap">
                                            <div class="dot">
                                                <div class="count">2</div>
                                            </div>
                                        </div>
                                        <div class="content">
                                            <h5 class="heading heading-h5">Approaching</h5>
                                            <p class="bk_pra">quickly translate the visions into reality and patent possible approaches.</p>
                                        </div>
                                    </div>
                                    <div class="item-grid mt--40 move-up-x wow" data-wow-delay=".13s">
                                        <div class="line"></div>
                                        <div class="dot-wrap">
                                            <div class="dot">
                                                <div class="count">3</div>
                                            </div>
                                        </div>
                                        <div class="content">
                                            <h5 class="heading heading-h5">Adjusting</h5>
                                            <p class="bk_pra">release approaches out into the world, submit it to criticism &amp; adaptation.</p>
                                        </div>
                                    </div>
                                    <!--]-->
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <Footer />
            </div>
            <!--]-->
        </div>
    </div>

</template>

<!-- <script>
export default {
  mounted() {
    this.loadExternalScript();
  },
  methods: {
    async loadExternalScript() {
      const module = await import('');
      // Lakukan sesuatu dengan modul yang diimpor, jika diperlukan
    }
  }
}
</script> -->
<style scoped>
  /* Add your custom styles here */
</style>
